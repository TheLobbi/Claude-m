...
const ONENOTE_5K_HINT = "OneDrive document libraries that back this user OneNote contain more than 5,000 items, which Graph refuses to enumerate. Workarounds: (1) call with a specific notebookId/sectionId rather than tenant-wide listing, (2) use the OneNote desktop app to consolidate notebooks, or (3) use search_query with entityTypes=[driveItem] to find pages by title.";
function isOneNote5kError(err) {
    const msg = String(err?.message ?? "").toLowerCase();
    return (msg.includes("5,000 onenote items") ||
        msg.includes("5000 onenote items") ||
        /onenote.*cannot be queried/i.test(msg));
}
function rewrap5k(err) {
    const wrapped = new Error("OneNote API limit exceeded: " + ONENOTE_5K_HINT + " (Original Graph error: " + (err?.message ?? err) + ")");
    wrapped.cause = err;
    wrapped.code = "OneNoteItemLimitExceeded";
    return wrapped;
}
