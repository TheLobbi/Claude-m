...
const userTarget = z
    .string()
    .optional()
    .describe("Target user UPN or id (the meeting organizer). Omit for /me (delegated). " +
    "App-only requires ApplicationAccessPolicy grant for that user.");
function meetingScope(user, meetingId) {
...
export const transcripts_list = makeTool({
    ...
    description: "List transcript metadata for a scheduled meeting. Each entry has id, createdDateTime, transcriptContentUrl. " +
    ...
    call: async ({ meetingId, user, ...opts }, client) => {
        return shapeListResponse(await listRequest(client, `${meetingScope(user, meetingId)}/transcripts`, opts).get());
    },
});
...
export const transcripts_getContent = makeTool({
    ...
    description: "Fetch a transcript's content. Default format is WebVTT (`text/vtt`) — pass format='text/plain' for a stripped plain-text version (Graph parses the VTT and returns just utterances). " +
...
export const transcripts_listByOrganizer = makeTool({
    ...
    description: "App-only: list every transcript for meetings organized by a specific user. Useful for batch processing (morning report, weekly roundup). Filter via $filter on createdDateTime to scope to a date range.",
    mode: "app",
    inputSchema: ListOptionsSchema.extend({
...
export const recordings_list = makeTool({
    ...
    description: "List recording metadata for a scheduled meeting. Each entry has id, createdDateTime, recordingContentUrl. " +
    ...
    call: async ({ meetingId, user, ...opts }, client) => {
        return shapeListResponse(await listRequest(client, `${meetingScope(user, meetingId)}/recordings`, opts).get());
    },
});
...
export const recordings_getDownloadUrl = makeTool({
    ...
    description: "Return the recordingContentUrl (a short-lived pre-authenticated URL to the mp4). Avoid streaming the whole file through the MCP — for large recordings, hand the URL back to the user/agent and let them download directly.",
    mode: "delegated",
    inputSchema: z.object({
        meetingId: z.string(),
        recordingId: z.string(),
    ...
    call: async ({ meetingId, recordingId, user }, client) => {
        ...
        if (!meta?.recordingContentUrl) {
            throw new Error("Recording has no recordingContentUrl. Either the recording isn't ready yet, or your permission scope doesn't include OnlineMeetingRecording.Read.All.");
        }
        return {
...
export const recordings_listByOrganizer = makeTool({
    ...
    description: "App-only: list every recording for meetings organized by a specific user. Companion to transcripts_listByOrganizer.",
    mode: "app",
    inputSchema: ListOptionsSchema.extend({
        user: z.string().describe("Organizer UPN or id (required in app-only mode)."),
    }),
