 ...
 */
import { z } from "zod";
import { ListOptionsSchema, listRequest, makeTool, shapeListResponse } from "./_helpers.js";
const userTarget = z
    .string()
    .optional()
...
export const calendar_createEvent = makeTool({
    ...
    call: async ({ user, subject, start, end, timeZone, body, bodyType, location, attendees, isOnlineMeeting, showAs, importance, reminderMinutesBeforeStart, categories }, client) => {
        ...
        if (body)
            payload.body = { contentType: bodyType, content: body };
        if (location)
        ...
        if (reminderMinutesBeforeStart !== undefined)
            payload.reminderMinutesBeforeStart = reminderMinutesBeforeStart;
...
export const calendar_updateEvent = makeTool({
    ...
    call: async ({ eventId, user, subject, start, end, timeZone, body, bodyType, location }, client) => {
        ...
        if (start)
            payload.start = { dateTime: start, timeZone: timeZone ?? "UTC" };
        if (end)
            payload.end = { dateTime: end, timeZone: timeZone ?? "UTC" };
        if (body)
            payload.body = { contentType: bodyType ?? "html", content: body };
        if (location)
            payload.location = { displayName: location };
