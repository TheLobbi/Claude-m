...
export const planner_createPlan = makeTool({
    ...
    call: async ({ groupId, title }, client) => client.api(`/planner/plans`).post({ owner: groupId, title }),
});
...
export const planner_createBucket = makeTool({
    ...
    call: async ({ planId, name, orderHint }, client) => client.api(`/planner/buckets`).post({ planId, name, orderHint: orderHint ?? " !" }),
});
export const planner_listTasks = makeTool({
    ...
    call: async ({ planId, bucketId, ...opts }, client) => {
        if (!planId && !bucketId)
            throw new Error("Provide planId or bucketId");
...
export const planner_createTask = makeTool({
    ...
    call: async (input, client) => {
        const payload = { planId: input.planId, title: input.title };
        if (input.bucketId)
            payload.bucketId = input.bucketId;
        ...
        if (input.percentComplete !== undefined)
            payload.percentComplete = input.percentComplete;
        if (input.assigneeIds?.length) {
            payload.assignments = Object.fromEntries(input.assigneeIds.map((id) => [
                id,
...
export const planner_updateTask = makeTool({
    ...
    call: async ({ taskId, etag, patch }, client) => client.api(`/planner/tasks/${taskId}`).header("If-Match", etag).patch(patch),
});
