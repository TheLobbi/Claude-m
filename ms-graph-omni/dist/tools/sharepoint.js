...
export const sites_search = makeTool({
    ...
    call: async ({ query, top }, client) => client.api(`/sites`).query({ search: query, $top: String(top) }).get(),
});
...
export const sites_listLists = makeTool({
    ...
    call: async ({ siteId, ...opts }, client) => applyListOptions(client.api(`/sites/${siteId}/lists`), opts).get(),
});
export const sites_listListItems = makeTool({
    ...
    call: async ({ siteId, listId, ...opts }, client) => applyListOptions(client.api(`/sites/${siteId}/lists/${listId}/items`), opts).get(),
});
export const sites_createListItem = makeTool({
    ...
    call: async ({ siteId, listId, fields }, client) => client.api(`/sites/${siteId}/lists/${listId}/items`).post({ fields }),
});
export const sites_updateListItem = makeTool({
    ...
    call: async ({ siteId, listId, itemId, fields }, client) => client.api(`/sites/${siteId}/lists/${listId}/items/${itemId}/fields`).patch(fields),
});
