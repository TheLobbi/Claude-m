...
const FEED_DIR = process.env.MSGO_CHANGEFEED_DIR ??
...
function feedPath(subscriptionId) {
...
export function recordNotification(rec) {
    try {
        ensureDir();
        ...
        logger.warn({ err: e.message, sub: rec.subscriptionId }, "changefeed write failed");
    }
}
function rotateIfNeeded(subscriptionId) {
    try {
        const p = feedPath(subscriptionId);
        const raw = readFileSync(p, "utf-8");
        const lines = raw.split("\n").filter((l) => l.length > 0);
        if (lines.length <= MAX_LINES_PER_SUB)
            return;
...
export function listFeedSubscriptions() {
    try {
        ensureDir();
...
export function readRecent(limit = 50, subscriptionId) {
    try {
        ensureDir();
        ...
        logger.warn({ err: e.message }, "changefeed read failed");
        return [];
    }
}
export function clearFeed(subscriptionId) {
    try {
        const p = feedPath(subscriptionId);
        if (existsSync(p))
            unlinkSync(p);
