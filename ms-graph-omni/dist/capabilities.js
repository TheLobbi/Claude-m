...
export const ALL_CAPABILITY_PACKS = [
    "internal-full",
    "baseline-read",
    "collaboration",
    "meetings",
...
export const CLIENT_INSTALLABLE_CAPABILITY_PACKS = ALL_CAPABILITY_PACKS.filter((pack) => pack !== "internal-full");
const __dirname = dirname(fileURLToPath(import.meta.url));
const CAPABILITY_PACK_DIR = resolve(__dirname, "..", "config", "capability-packs");
const manifestCache = new Map();
function readCapabilityPackManifest(pack) {
...
export function getCapabilityPackManifest(pack) {
...
export function requiredResourcesForPacks(packs) {
    return [...new Set(packs.flatMap((pack) => getCapabilityPackManifest(pack).resources))];
}
export function prerequisitesForPacks(packs) {
    return [...new Set(packs.flatMap((pack) => getCapabilityPackManifest(pack).prerequisites))];
}
export function capabilityPacksForEnvironment(environment) {
    return environment === "client" ? ["baseline-read"] : ["internal-full"];
}
export function effectiveCapabilityPacks(tenant) {
    return tenant.capabilityPacks?.length
        ? tenant.capabilityPacks
        : capabilityPacksForEnvironment(tenant.environment);
}
export function enabledNamespaces(tenant) {
    const fromPacks = effectiveCapabilityPacks(tenant).flatMap((pack) => getCapabilityPackManifest(pack).namespaces);
    const explicit = tenant.allowedNamespaces ?? [];
    return [...new Set([...fromPacks, ...explicit])];
}
export function validateTenantCapabilityProfile(tenant) {
    ...
    if (tenant.environment === "client" && packs.includes("internal-full")) {
        throw new Error(`Capability pack internal-full cannot be enabled for '${tenant.slug}' because it is a client tenant profile.`);
    }
    ...
    if (tenant.environment === "client" && nonBaselineClientPacks.length > 0 && !tenant.consent?.evidenceRef) {
        throw new Error(`Client tenant '${tenant.slug}' enables ${nonBaselineClientPacks.join(", ")}; consent.evidenceRef is required.`);
    }
}
export function namespaceForToolName(toolName) {
...
export function assertTenantAllowsNamespace(tenant, toolName, namespace = namespaceForToolName(toolName)) {
    const allowed = enabledNamespaces(tenant);
    if (!allowed.includes(namespace)) {
        throw new Error(`tenantSlug='${tenant.slug}' cannot run tool '${toolName}': namespace '${namespace}' is not enabled. ` +
