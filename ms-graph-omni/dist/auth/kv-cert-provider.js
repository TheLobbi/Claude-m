...
function certificateCacheKey(vaultUrl, certName) {
    return `${vaultUrl}|${certName}`;
}
function pfxToPem(pfxBytes) {
    ...
    try {
        ...
        for (const openssl of OPENSSL_CANDIDATES) {
            try {
                ...
                if (!keyMatch || !certMatch)
                    throw new Error("openssl PEM output missing key or cert");
        ...
        throw new Error(`openssl pkcs12 conversion failed for all candidates. ${failures.join(" | ")}`);
    }
    finally {
        rmSync(dir, { recursive: true, force: true });
    }
}
export async function getCertificate(vaultUrl, certName) {
