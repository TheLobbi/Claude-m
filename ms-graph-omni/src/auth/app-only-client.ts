 ...
 */
import { ConfidentialClientApplication, LogLevel, type ICachePlugin } from "@azure/msal-node";
import { existsSync, mkdirSync } from "node:fs";
import { join, dirname } from "node:path";
import { getCertificate } from "./kv-cert-provider.js";
import { logger } from "../logger.js";

const clientPool = new Map<string, ConfidentialClientApplication>();
...
async function buildAppCachePlugin(clientId: string): Promise<ICachePlugin | undefined> {
  ...
  try {
    ...
    logger.warn(
      { err: (e as Error).message, cacheFile },
      "msal-node-extensions protected cache unavailable for app-only auth; using memory-only cache"
    );
    return undefined;
  }
}
...
async function getClient(
  ...
  const { privateKey, thumbprint } = await getCertificate(
    vaultUrl,
    certName
  );
    ...
    system: {
      loggerOptions: {
        ...
        loggerCallback: (level: LogLevel, message: string) => {
          ...
          else if (level === LogLevel.Warning) logger.warn(message);
        },
  ...
  const client = new ConfidentialClientApplication(
    cachePlugin ? { ...clientConfig, cache: { cachePlugin } } : clientConfig
  );
