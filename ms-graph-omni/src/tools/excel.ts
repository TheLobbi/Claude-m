...
function wbPath(t: z.infer<typeof wbTarget>): string {
  if (t.type === "me") return `/me/drive/items/${t.itemId}/workbook`;
  if (t.type === "user") return `/users/${t.userId}/drive/items/${t.itemId}/workbook`;
  return t.driveId
...
export const excel_addRowsToTable = makeTool({
  ...
  call: async ({ target, tableNameOrId, values }, client) =>
    client.api(`${wbPath(target)}/tables/${encodeURIComponent(tableNameOrId)}/rows/add`).post({ values }),
});
...
export const excel_createSession = makeTool({
  ...
  call: async ({ target, persistChanges }, client) =>
    client.api(`${wbPath(target)}/createSession`).post({ persistChanges }),
});
