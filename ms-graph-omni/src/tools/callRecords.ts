...
export const callRecords_list = makeTool({
  ...
  description:
    "List call records (calls + meetings that ended in the last 30 days). " +
...
export const callRecords_get = makeTool({
  ...
  description:
    "Get a single call record by id with optional expansion. Default expand fetches sessions + participants_v2; pass expand=[] to skip expansion.",
  mode: "app",
...
export const callRecords_listSessions = makeTool({
  ...
  description:
    "List sessions within a call record. Each session is a sub-call (e.g. one user joining/leaving a meeting). Use $expand=segments for the segment-by-segment quality data.",
  mode: "app",
  ...
  call: async ({ callRecordId, ...opts }, client) => {
    return shapeListResponse(
      await listRequest(client, `/communications/callRecords/${callRecordId}/sessions`, opts).get()
    );
  },
});
...
export const callRecords_listParticipants = makeTool({
  ...
  description:
    "List participants_v2 of a call record. Each entry includes identity (user, phone, guest), join/leave timestamps, and feedback when available.",
  mode: "app",
  inputSchema: ListOptionsSchema.extend({
    callRecordId: z.string(),
  }),
  call: async ({ callRecordId, ...opts }, client) => {
    return shapeListResponse(
      await listRequest(client, `/communications/callRecords/${callRecordId}/participants_v2`, opts).get()
    );
  },
});
...
export const callRecords_getPstnCalls = makeTool({
  ...
  description:
    "Get PSTN (calling-plan) call detail records as a tabular projection. Useful for billing/usage reports. Date range is required and must be within 90 days. Requires CallRecord-PstnCalls.Read.All in addition to CallRecords.Read.All.",
  mode: "app",
  inputSchema: z.object({
  ...
  call: async ({ fromDateTime, toDateTime }, client) => {
    return await client
      .api(
        `/communications/callRecords/getPstnCalls(fromDateTime=${fromDateTime},toDateTime=${toDateTime})`
      )
      .get();
  },
});
...
export const callRecords_getDirectRoutingCalls = makeTool({
  ...
  description:
    "Get Direct Routing call detail records as a tabular projection. Same permission notes as getPstnCalls.",
  mode: "app",
  inputSchema: z.object({
    fromDateTime: z.string().describe("ISO 8601 (inclusive)."),
    toDateTime: z.string().describe("ISO 8601 (exclusive)."),
  ...
  call: async ({ fromDateTime, toDateTime }, client) => {
    return await client
      .api(
        `/communications/callRecords/getDirectRoutingCalls(fromDateTime=${fromDateTime},toDateTime=${toDateTime})`
      )
      .get();
  },
});
