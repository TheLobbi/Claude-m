...
export const todo_listTasks = makeTool({
  ...
  call: async ({ listId, ...opts }, client) => {
    ...
    try {
      ...
      if (!looksLikeQueryQuirk) throw err;
...
export const todo_createTask = makeTool({
  ...
  call: async ({ listId, title, body, dueDateTime, importance, reminderDateTime }, client) => {
    ...
    if (dueDateTime) payload.dueDateTime = { dateTime: dueDateTime, timeZone: "UTC" };
    if (importance) payload.importance = importance;
    if (reminderDateTime) payload.reminderDateTime = { dateTime: reminderDateTime, timeZone: "UTC" };
...
export const todo_updateTask = makeTool({
  ...
  call: async ({ listId, taskId, patch }, client) =>
...
export const todo_completeTask = makeTool({
  ...
  call: async ({ listId, taskId }, client) =>
    client.api(`/me/todo/lists/${listId}/tasks/${taskId}`).patch({ status: "completed" }),
});
