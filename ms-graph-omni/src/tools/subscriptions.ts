...
export const subscriptions_list = makeTool({
  ...
  description:
    "List active change-notification subscriptions visible to the calling app/user. " +
...
const RESOURCE_PRESETS = z
  ...
  .describe(
    "Common resource paths. Pick one of these or pass a custom string in `resource` — the preset is just a hint."
  );
...
export const subscriptions_create = makeTool({
  ...
  inputSchema: z.object({
    ...
    lifecycleNotificationUrl: z
      ...
      .optional()
      .describe(
        "HTTPS endpoint for reauthorizationRequired / missed / subscriptionRemoved notifications. STRONGLY recommended for long-lived subscriptions."
      ),
    expirationDateTime: z
      .string()
      .optional()
      .describe(
        "ISO 8601. Max ~71h for most resources, 1h for chatMessage. Default: 60h from now (safe for mail/event/drive)."
      ),
    clientState: z
      .string()
      .optional()
      .describe(
        "Random string echoed back in every notification. Use to verify authenticity. Auto-generated when omitted (UUID)."
      ),
    includeResourceData: z
      .boolean()
      .default(false)
      .describe(
        "If true, Graph encrypts and includes the resource payload in each notification (Outlook + Teams only). Requires encryptionCertificate + encryptionCertificateId."
      ),
  ...
  call: async (input, client) => {
    ...
    if (input.lifecycleNotificationUrl) payload.lifecycleNotificationUrl = input.lifecycleNotificationUrl;
    if (input.includeResourceData) {
      payload.includeResourceData = true;
      if (!input.encryptionCertificate || !input.encryptionCertificateId) {
        throw new Error(
          "includeResourceData=true requires encryptionCertificate + encryptionCertificateId."
        );
      }
...
export const subscriptions_renew = makeTool({
  ...
  description:
    "Extend a subscription's expirationDateTime. Required before each subscription expires (within ~71h). " +
