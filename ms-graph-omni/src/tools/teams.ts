...
export const teams_listChannels = makeTool({
  ...
  call: async ({ teamId, ...opts }, client) =>
    applyListOptions(client.api(`/teams/${teamId}/channels`), opts).get(),
});
...
export const teams_createChannel = makeTool({
  ...
  call: async ({ teamId, displayName, description, membershipType }, client) =>
    client.api(`/teams/${teamId}/channels`).post({ displayName, description, membershipType }),
});
...
export const teams_listChannelMessages = makeTool({
  ...
  call: async ({ teamId, channelId, ...opts }, client) =>
    applyListOptions(client.api(`/teams/${teamId}/channels/${channelId}/messages`), opts).get(),
});
...
export const teams_sendChatMessage = makeTool({
  ...
  call: async ({ chatId, body, bodyType }, client) =>
    client.api(`/chats/${chatId}/messages`).post({ body: { content: body, contentType: bodyType } }),
});
...
export const teams_createChat = makeTool({
  ...
  call: async ({ chatType, userIds, topic }, client) => {
    ...
    const payload: any = { chatType, members };
