...
export const security_listIncidents = makeTool({
  ...
  call: async (opts, client) =>
    shapeListResponse(await listRequest(client, "/security/incidents", opts).get()),
});
...
export const security_listAlerts = makeTool({
  ...
  inputSchema: ListOptionsSchema,
  call: async (opts, client) =>
    shapeListResponse(await listRequest(client, "/security/alerts_v2", opts).get()),
});
...
export const security_listSecureScores = makeTool({
  ...
  inputSchema: ListOptionsSchema,
  call: async (opts, client) =>
    shapeListResponse(await listRequest(client, "/security/secureScores", opts).get()),
});
