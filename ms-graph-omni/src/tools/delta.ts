...
interface DeltaResult {
  /** Items added / changed / deleted since last sync (or all current items on first call). */
  value: unknown[];
  /** Number of items returned in this sync. */
  changeCount: number;
  /** True if a stored token was used (incremental); false if this was a fresh sync. */
  resumed: boolean;
  /** True when we walked all pages and persisted a fresh deltaLink. */
  complete: boolean;
  /** When complete=false, the in-progress nextLink we saved for the next call. (We do NOT persist these — caller must call again to drain.) */
  nextLink?: string;
  /** Hint string for the LLM when the previous token was invalidated. */
  resyncReason?: string;
}

interface RunDeltaOpts {
  client: Client;
  tenantSlug: string;
  resource: string;
  scope: string;
  /** Function building the very-first delta URL when no token is stored. */
  freshUrl: () => string;
  /** Maximum pages to walk per call. Caller can re-invoke if not complete. Default 5 keeps responses bounded. */
  maxPages?: number;
}

async function runDelta(opts: RunDeltaOpts): Promise<DeltaResult> {
  ...
  while (true) {
    ...
    pages += 1;
    const value = Array.isArray(response?.value) ? (response.value as unknown[]) : [];
    merged.push(...value);
    lastDeltaLink = extractDeltaLink(response);
    lastNextLink = extractNextLink(response);

    if (lastDeltaLink) break; // sync complete
    if (!lastNextLink) break;  // no more data, no deltaLink (shouldn't happen but be safe)
...
export const mail_delta = makeTool({
  name: "mail_delta",
  description:
    "Incremental change tracking for messages in a mail folder. First call returns all messages and a deltaLink; subsequent calls return only changes (added / updated / deleted) since last invocation. " +
...
export const calendar_delta = makeTool({
  name: "calendar_delta",
  description:
    "Incremental change tracking for calendar events in a date window. Note: calendarView/delta REQUIRES startDateTime + endDateTime parameters on the FIRST call; resumes via deltaLink thereafter. Default window = next 30 days from invocation.",
  mode: "delegated",
  inputSchema: z.object({
  ...
  call: async ({ user, startDateTime, endDateTime, maxPages, tenantSlug }, client) => {
    ...
    return await runDelta({
      ...
      scope: `${user ?? "me"}`,
      freshUrl: () =>
        `${base}/calendarView/delta?startDateTime=${encodeURIComponent(start)}&endDateTime=${encodeURIComponent(end)}`,
      maxPages,
    });
  },
});
...
export const drive_delta = makeTool({
  name: "drive_delta",
  description:
    "Incremental change tracking for OneDrive / SharePoint drive items. Default scope is /me/drive/root; pass userId to target another user's drive (app-only) or {siteId, driveId} for a SharePoint document library.",
  mode: "delegated",
  inputSchema: z.object({
...
export const users_delta = makeTool({
  name: "users_delta",
  description:
    "Tenant-wide directory sync of user objects. App-only (delegated unsupported here). Useful for keeping a local user mirror in sync without re-listing the entire directory each run.",
  mode: "app",
  inputSchema: z.object({
...
export const groups_delta = makeTool({
  name: "groups_delta",
  description:
    "Tenant-wide directory sync of group objects (security, M365, distribution). App-only. Use $expand=members in a custom select string to also track group membership changes.",
  mode: "app",
  inputSchema: z.object({
  ...
  call: async ({ select, expandMembers, maxPages, tenantSlug }, client) => {
    ...
    if (expandMembers) params.push("$expand=members");
...
export const todo_delta = makeTool({
  name: "todo_delta",
  description:
    "Incremental change tracking for tasks in a Microsoft To Do list. Useful for keeping a local task mirror in sync.",
  mode: "delegated",
  inputSchema: z.object({
...
export const delta_clear = makeTool({
  name: "delta_clear",
  description:
    "Drop a stored delta token to force the next call to start a fresh full sync. Use when you suspect drift (e.g. someone added properties via another tool, or the user wants to re-pull everything).",
  mode: "app",
  inputSchema: z.object({
