...
const userTarget = z
  ...
  .optional()
  .describe(
    "Target mailbox UPN or id (e.g. user@thelobbi.io). Omit for /me (delegated)."
  );
...
export const mail_sendMail = makeTool({
  ...
  call: async ({ user, to, cc, bcc, subject, body, bodyType, saveToSentItems, attachments, replyTo }, client) => {
    ...
    if (replyTo?.length) message.replyTo = replyTo.map((a) => ({ emailAddress: { address: a } }));
...
export const mail_createDraft = makeTool({
  ...
  call: async ({ user, ...rest }, client) => {
    ...
    if (rest.to?.length) payload.toRecipients = rest.to.map((a) => ({ emailAddress: { address: a } }));
    if (rest.cc?.length) payload.ccRecipients = rest.cc.map((a) => ({ emailAddress: { address: a } }));
    if (rest.bcc?.length) payload.bccRecipients = rest.bcc.map((a) => ({ emailAddress: { address: a } }));
...
export const mail_listFolders = makeTool({
  ...
  call: async ({ user, includeHidden, ...opts }, client) => {
    ...
    if (includeHidden) req = req.query({ includeHiddenFolders: "true" });
    return await req.get();
  },
});
