...
export const omi_startSubscription = makeResourceTool({
  ...
  call: async ({ contentType }, client, tenant) =>
    client.post(`/api/v1.0/${tenant.tenantId}/activity/feed/subscriptions/start`, undefined, {
      query: { contentType },
    }),
});
...
export const omi_listSubscriptions = makeResourceTool({
  ...
  call: async (_input, client, tenant) =>
    client.get(`/api/v1.0/${tenant.tenantId}/activity/feed/subscriptions/list`),
});
...
export const omi_listContent = makeResourceTool({
  ...
  call: async ({ contentType, startTime, endTime }, client, tenant) =>
    client.get(`/api/v1.0/${tenant.tenantId}/activity/feed/subscriptions/content`, {
      contentType,
      startTime,
      endTime,
    }),
