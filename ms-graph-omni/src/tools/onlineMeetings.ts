...
const userTarget = z
  ...
  .optional()
  .describe(
    "Target user UPN or id (e.g. user@thelobbi.io). Omit for /me (delegated). " +
...
function meetingsBase(user: string | undefined): string {
...
export const onlineMeetings_list = makeTool({
  ...
  description:
    "List a user's scheduled online meetings. Supports $filter and $orderby. " +
...
export const onlineMeetings_get = makeTool({
  ...
  description:
    "Get a single online meeting by id. The id is the onlineMeeting object id (not the joinMeetingId or videoTeleconferenceId). " +
...
export const onlineMeetings_listByJoinWebUrl = makeTool({
  ...
  description:
    "Look up a meeting by its joinWebUrl. Useful when you have the meeting URL from a calendar event but not the meeting id. Returns up to 1 meeting (joinWebUrls are unique per organizer).",
  mode: "delegated",
  inputSchema: z.object({
...
export const onlineMeetings_create = makeTool({
  ...
  description:
    "Create an online (Teams) meeting. Returns the meeting object including joinWebUrl, joinMeetingId, and the dial-in conference id (videoTeleconferenceId).",
  mode: "delegated",
  inputSchema: z.object({
    user: userTarget,
    subject: z.string(),
  ...
  call: async (
    ...
    if (allowedPresenters) payload.allowedPresenters = allowedPresenters;
    if (allowAttendeeToEnableCamera != null) payload.allowAttendeeToEnableCamera = allowAttendeeToEnableCamera;
    if (allowAttendeeToEnableMic != null) payload.allowAttendeeToEnableMic = allowAttendeeToEnableMic;
    if (allowMeetingChat) payload.allowMeetingChat = allowMeetingChat;
    if (isEntryExitAnnounced != null) payload.isEntryExitAnnounced = isEntryExitAnnounced;
    if (lobbyBypassScope) payload.lobbyBypassSettings = { scope: lobbyBypassScope };
    if (recordAutomatically != null) payload.recordAutomatically = recordAutomatically;
