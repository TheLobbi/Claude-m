/**
 * Translate raw Microsoft Graph (and MSAL) errors into a structured payload
 * the LLM can reason about.
 *
 * Why: server.ts used to return `Error in <tool>: <err.message>`, which leaked raw
 * Graph strings like "Resource 'manager' does not exist or one of its queried
 * reference-property objects are not present" or
 * "Authorization_RequestDenied: Insufficient privileges to complete the operation".
 * Claude has to reverse-engineer the recovery from those.
 *
 * After translation each error becomes:
 *   {
 *     code:           "insufficientScope" | "tooManyRequests" | "preconditionFailed" | ...
 *     statusCode:     403,
 *     summary:        "Missing application permission for this operation.",
 *     recoveryHint:   "Grant the listed scopes via admin consent and retry.",
 *     requiredScopes: ["Tasks.ReadWrite.All"],   // when extractable
 *     retryAfterMs:   3000,                       // when 429 with Retry-After header
 *     isRetryable:    true,
 *     originalMessage: "...",                    // raw, for debugging
 *     originalCode:    "Authorization_RequestDenied",
 *   }
 *
 * Sourced against:
...
export type TranslatedErrorCode =
  | "invalidAuthentication"
  | "expiredAuthentication"
  | "insufficientScope"
  | "appAccessPolicyMissing"
  | "itemNotFound"
  | "preconditionFailed"
  | "conflict"
  | "tooManyRequests"
 ...
 */
export function translateGraphError(err: unknown, ctx: TranslateContext = {}): TranslatedError {
  const e = err as Partial<GraphError> & {
    statusCode?: number;
    status?: number;
    code?: string;
    message?: string;
    ...
    response?: { headers?: Headers | Record<string, string>; status?: number };
  };
  ...
  if (statusCode === 412) {
    return {
      ...
      recoveryHint:
        "Re-fetch the resource (e.g., planner_getTask) to obtain the latest @odata.etag, then retry the patch with that value in If-Match.",
      isRetryable: false,
    };
  }
  ...
  if (statusCode === 409) {
    return {
      ...
      recoveryHint:
        "Check whether a resource with the same name already exists, or set @microsoft.graph.conflictBehavior to 'replace' or 'rename' on file uploads.",
      isRetryable: false,
    };
  }
  ...
  if (statusCode === 404) {
    return {
      ...
      recoveryHint:
        "Verify the id / path is correct. Common cause: targeting /me/manager when no manager is assigned, or using a stale id after a delete.",
      isRetryable: false,
    };
  }
  ...
  if (statusCode === 401) {
    const code: TranslatedErrorCode = /expired|lifetime/i.test(originalMessage)
      ? "expiredAuthentication"
      : "invalidAuthentication";
    return {
      ...base,
  ...
  if (statusCode === 403) {
    ...
    if (
      /no application access policy found for this app/i.test(originalMessage) ||
      ...
      return {
        ...
        summary:
          "App-only access to this user's online meetings / transcripts requires an ApplicationAccessPolicy that is not configured for this user.",
        recoveryHint:
          "A tenant admin must run scripts/grant-meeting-access-policy.ps1 (Skype for Business PowerShell). Propagation can take up to 30 minutes.",
        isRetryable: false,
      };
    }
  ...
  if (statusCode === 400) {
    ...
    if (originalCode === "syncStateNotFound" || /sync\s*state\s*not\s*found/i.test(originalMessage)) {
      return {
        ...
        recoveryHint:
          "Restart the delta sync from scratch by calling the *delta endpoint without a $deltatoken parameter.",
        isRetryable: false,
      };
    }
    if (originalCode === "syncStateInvalid" || /sync\s*state\s*invalid/i.test(originalMessage)) {
      return {
        ...
        recoveryHint:
          "Discard the saved deltaLink and restart the delta sync. The first follow-up call will return a fresh deltaLink.",
        isRetryable: false,
      };
    }
    ...
    return {
      ...
      recoveryHint:
        "Check parameter names, values, and combinations. Some endpoints (e.g. /teams/{id}/channels) reject $top — see the tool description.",
      isRetryable: false,
    };
  }
...
function pickStatus(e: { statusCode?: number; status?: number; response?: { status?: number } } | undefined): number | null {
  if (!e) return null;
...
function pickStringField(obj: unknown, field: string): string | undefined {
  if (typeof obj !== "object" || obj === null) return undefined;
  const v = (obj as Record<string, unknown>)[field];
  return typeof v === "string" ? v : undefined;
}

function extractInnerCode(body: unknown): string | undefined {
...
function parseRetryAfter(headers: Headers | Record<string, string> | undefined): number | undefined {
...
function extractRequiredScopes(
  e: { headers?: Headers | Record<string, string>; response?: { headers?: Headers | Record<string, string> } } | undefined,
  message: string
): string[] | undefined {
