...
async function main() {
  ...
  server.registerResource(
    ...
    "msgo://stream/recent",
    {
      ...
      description:
        "Last 50 change notifications received by the webhook receiver, newest first. Run `pnpm receiver` and configure the public notificationUrl to populate this feed.",
  ...
  server.registerResource(
    "change-feed-by-subscription",
    new ResourceTemplate("msgo://stream/sub/{subscriptionId}", {
      list: async () => ({
    ...
    }),
    {
      ...
      description:
        "Buffered change notifications for a single Graph subscription. The id matches what subscriptions_create returned.",
      mimeType: "application/json",
    },
    async (uri, { subscriptionId }) => {
      const subId = Array.isArray(subscriptionId) ? subscriptionId[0] : subscriptionId;
      const items = readRecent(200, subId as string);
      return {
        contents: [
          {
            uri: uri.href,
  ...
  for (const tool of tools) {
    ...
    (server.registerTool as Function)(
      ...
      async (parsedInput: unknown) => {
        return withObservability(
          { toolName: tool.name, mode: tool.mode, annotations, inputForAudit: parsedInput },
          async () => {
            try {
              const result = await tool.handler(parsedInput as never);
              const text =
              ...
              if (tool.outputSchema && result !== undefined && result !== null && typeof result === "object") {
                ...
                logger.warn(
                  { tool: tool.name, issues: parsed.error.issues },
                  "outputSchema validation failed; returning text only"
                );
              }
...
main().catch((err) => {
  logger.fatal({ err: (err as Error).message, stack: (err as Error).stack }, "fatal startup error");
  process.exit(1);
});
