...
function tokenPath(tenantSlug: string, resource: string, scope: string): string {
  return join(STORE_DIR, `${safeSlug(tenantSlug)}-${safeSlug(resource)}-${safeSlug(scope)}.json`);
}

export function readDelta(tenantSlug: string, resource: string, scope: string): DeltaToken | null {
  try {
    ensureDir();
    const p = tokenPath(tenantSlug, resource, scope);
    if (!existsSync(p)) return null;
    return JSON.parse(readFileSync(p, "utf-8")) as DeltaToken;
  } catch (e) {
    logger.warn({ err: (e as Error).message }, "delta store read failed; treating as fresh");
    return null;
  }
}
...
export function writeDelta(
  ...
  try {
    ...
    logger.warn({ err: (e as Error).message }, "delta store write failed; next call will start fresh");
  }
}

/** Drop a stored delta token. Call this when Graph returns syncStateNotFound / syncStateInvalid. */
export function clearDelta(tenantSlug: string, resource: string, scope: string): void {
  try {
    const p = tokenPath(tenantSlug, resource, scope);
...
export function extractDeltaLink(response: unknown): string | null {
  if (typeof response !== "object" || response === null) return null;
  const v = (response as Record<string, unknown>)["@odata.deltaLink"];
  return typeof v === "string" ? v : null;
}

/** Extract the @odata.nextLink (intermediate page during a multi-page delta sync). */
export function extractNextLink(response: unknown): string | null {
  if (typeof response !== "object" || response === null) return null;
  const v = (response as Record<string, unknown>)["@odata.nextLink"];
  return typeof v === "string" ? v : null;
}
