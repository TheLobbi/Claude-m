...
import { recordNotification, listFeedSubscriptions, type NotificationRecord } from "./changeFeed.js";
...
interface GraphChangeNotificationsBody {
  value?: Array<{
...
async function handleNotifications(req: IncomingMessage, res: ServerResponse, isLifecycle: boolean): Promise<void> {
  ...
  let body: GraphChangeNotificationsBody;
  try {
    const raw = await readBody(req);
    body = raw ? (JSON.parse(raw) as GraphChangeNotificationsBody) : {};
  } catch (e) {
    res.statusCode = 400;
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (path === "/notifications") return handleNotifications(req, res, false);
    if (path === "/lifecycle") return handleNotifications(req, res, true);
    if (path === "/health") return handleHealth(req, res);
    res.statusCode = 404;
    res.end("not found");
  } catch (e) {
    logger.error({ err: (e as Error).message, stack: (e as Error).stack }, "receiver request failed");
    res.statusCode = 500;
...
server.listen(PORT, () => {
  logger.info(
    { port: PORT, requireClientState: Boolean(EXPECTED_CLIENT_STATE) },
    "ms-graph-omni webhook receiver listening"
  );
